...
config_root = '../../configs/task2_update'
model_root = '../../../Task2_Update/models'
result_root = '../../../Task2_Update/results'
model_name = os.path.join(model_root, 'zhymir_base_model')
adv_model_name = os.path.join(model_root, 'zhymir_pgd_model')
result_name = os.path.join(result_root, 'zhymir_history')
adv_result_name = os.path.join(result_root, 'zhymir_adv_history')
...
# new_ensemble.train_adversarial_pgd(data_bs, labels, max_iter=20, epochs=1, norm='linf', eps=0.2)
# print(new_ensemble.evaluate(data_bs, labels))
# new_ensemble.save_model(model_name)
# new_ensemble.save_history(result_name)
# attack_args = {'attack': 'pgd', 'description': 'none', 'eps': 0.2, 'eps_step': 0.3/10, 'max_iter': 100,
#                'norm': 'linf', 'targeted': False, 'num_random_init': 0, 'random_eps': False}
...
# new_ensemble.save_model(adv_model_name)
# new_ensemble.save_history(adv_result_name)
